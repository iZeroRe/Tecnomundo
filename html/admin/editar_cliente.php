...
if (!isset($_GET['id']) || !is_numeric($_GET['id'])) {
    header('Location: clientes.php'); //Si es incorrecto regresamos
    exit;
}
$id_cliente_a_editar = $_GET['id'];


// COnsultas para el cliente seleccionado 
try {
    $stmt_cliente = $pdo->prepare("SELECT * FROM cliente WHERE id_cliente = ?");
    $stmt_cliente->execute([$id_cliente_a_editar]);
    $cliente = $stmt_cliente->fetch(PDO::FETCH_ASSOC);

    if (!$cliente) {
        header('Location: clientes.php'); // SI falla de nuevo a cliente.php
        exit;
    }

    // Obtener las empresas para la seleccion
...
<head>
    ...
    <title>Editar Cliente - Sistema de Administración</title>
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="num_direccion" id="num_direccion" class="form-control" 
                               value="<?php echo htmlspecialchars($cliente['num_direccion']); ?>" required>
                    </div>
