...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($id_cliente) || empty($id_productos)) {
        die("Error: Faltan datos del cliente o productos.");
    }
    ...
    try {
        ...
        for ($i = 0; $i < count($id_productos); $i++) {
            ...
            // Esto evita que dos personas vendan el mismo último producto a la vez
        ...
        $stmt_venta = $pdo->prepare("INSERT INTO venta (id_cliente, fecha_venta, total) VALUES (?, CURDATE(), ?)");
        $stmt_venta->execute([$id_cliente, $total_calculado]);
        ...
        for ($i = 0; $i < count($id_productos); $i++) {
            ...
            // Es más seguro recalcularlo aquí
        ...
        echo "<br><a href='venta_nueva.php'>Volver al formulario</a>";
        die();
    }
