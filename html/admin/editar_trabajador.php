...
$id_trabajador_a_editar = $_GET['id'];

try {
    // Consulta para el trabajador
    $stmt_trabajador = $pdo->prepare("SELECT * FROM trabajador WHERE id_trabajador = ?");
    $stmt_trabajador->execute([$id_trabajador_a_editar]);
    $trabajador = $stmt_trabajador->fetch(PDO::FETCH_ASSOC);

    // si no hay trabajador redirigir
    if (!$trabajador) {
        header('Location: trabajadores.php');
        exit;
    }

    // Obtener TODAS las empresas (para el dropdown) ---
...
<head>
    ...
    <title>Editar Trabajador - Sistema de Administración</title>
    ...
    <style>
        ...
        /* Todo el CSS de .sidebar, .main-content, .card, .form-grid, etc. va aqui */
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="nombre" id="nombre" class="form-control" 
                               value="<?php echo htmlspecialchars($trabajador['nombre']); ?>" required>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <small style="color: var(--color-texto-secundario); margin-top: 4px;">Deja en blanco si no quieres cambiar la contraseña.</small>
                    </div>
                    <div class="form-group">
                        <label for="rol">Rol</label>
                        <select name="rol" id="rol" class="form-control" required>
                            <option value="trabajador" <?php if ($trabajador['rol'] == 'trabajador') echo 'selected'; ?>>Trabajador</option>
                            <option value="admin" <?php if ($trabajador['rol'] == 'admin') echo 'selected'; ?>>Administrador</option>
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="email" name="correo" id="correo" class="form-control" 
                               value="<?php echo htmlspecialchars($trabajador['correo']); ?>" required>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="num_direccion" id="num_direccion" class="form-control" 
                               value="<?php echo htmlspecialchars($trabajador['num_direccion']); ?>" required>
                    </div>
                    
                    <div class="form-group">
                        <label for="especialidad">Especialidad</label>
                        <input type="text" name="especialidad" id="especialidad" class="form-control" 
                               value="<?php echo htmlspecialchars($trabajador['especialidad']); ?>" required>
                    </div>
                    <div class="form-group">
                        ...
                        <select name="id_empresa" id="id_empresa" class="form-control" required>
                            ...
                            <?php foreach ($empresas as $empresa): ?>
                                <option value="<?php echo $empresa['id_empresa']; ?>" 
                                    <?php if ($empresa['id_empresa'] == $trabajador['id_empresa']) echo 'selected'; ?>>
